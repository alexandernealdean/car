...
function Vehicle({ radius = 0.7, width = 1.2, height = -0.04, front = 1.3, back = -1.15, steer = 0.75, force = 2000, maxBrake = 1e5, ...props }) {
  ...
  useFrame((state) => {
    ...
    if (reset) {
      ...
      state.camera.position.set(chassis.current.position.x, chassis.current.position.y + 15, chassis.current.position.z + 15)
  ...
  return (
    <group ref={vehicle} position={[0, -0.4, 0]} rotation={[0, 0, 0]} scale={0.8}>
      <Cyberpunkcar ref={chassis} rotation={props.rotation} position={props.position} angularVelocity={props.angularVelocity} />
